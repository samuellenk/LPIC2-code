...
- mit `nc` sollte eine Verbindung zum Port 53 möglich sein
...
sudo apt-get update && sudo apt-get install -y bind9 bind9utils dnsutils
...
```bash
sudo rndc reload   # löst die Zone‑Übertragung vom Master aus
sudo rndc retransfer example.net # löst die Zone‑Übertragung vom Slave aus
```
